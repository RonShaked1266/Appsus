...
function addReview(bookId, review) {
    // let books = _loadFromStorage()
    // books = books.map(book => book.id === bookId ? review : book)
    // _saveToStorage(books)
...
function getOptions(search) {
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${search}`).then((res) => res.data.items)
}
